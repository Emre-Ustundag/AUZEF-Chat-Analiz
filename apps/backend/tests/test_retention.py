...
async def test_basarisiz_is_de_ham_dosyayi_siler(client: AsyncClient) -> None:
    """ADR §9: "başarı/hata/iptal fark etmeksizin"."""
    upload_id, key = await ready_upload(client)
    # Maliyet tavanı aşımı: iş `failed` ile biter.
    analysis_id = await start_analysis(client, upload_id, max_cost_usd=0.0000001)
