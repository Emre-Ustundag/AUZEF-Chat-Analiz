...
async def sweep_expired_uploads(
    ...
    RETENTION SÜRESİ DOLMAMIŞ RAPORU OLAN UPLOAD DA ATLANIR. Aynı CASCADE,
    terminal bir analizin raporunu da uçurur. Yalnızca upload yaşına bakmak,
    `report_retention_hours`'ı fiilen uygulanamaz kılıyordu: T=23s'te başlayıp
    T=23.5s'te tamamlanan bir analizin raporu, T=24s'te upload'ın süresi
    dolduğu için yarım saatlikken siliniyordu. Oysa ADR §9 raporu kendi
    penceresi boyunca tutmayı vaat ediyor. Rapor ömrü artık kendi yaşına
    bağlı, upload'ın yaşına değil.

    Koruma ÖLÇÜTÜ RAPORUN VARLIĞIDIR, terminal olmama değil. `report IS NOT
    NULL` yerine "terminal değilse veya tazeyse" demek, iptal edilmiş bir işin
    de upload'ı 24 saat kilitlemesi anlamına gelirdi — oysa iptalde saklanacak
    bir rapor yok ve o dosya ASIL süpürme hedefi.

    Koruma KALICI DEĞİL: rapor `report_retention_hours`'ı geçince
    `sweep_expired_analyses` onu siler, upload'ın koruması düşer ve bir
    sonraki geçişte (aynı `sweep` içinde analiz süpürmesi önce koştuğu için
    çoğu zaman aynı geçişte) toplanır. Erken silmeyi bir sızıntıyla
    değiştirmiyoruz.
    ...
    rapor_cutoff = _cutoff(settings.report_retention_hours, now)
    korunan_analiz = (
        select(Analysis.id)
        .where(
            Analysis.upload_id == Upload.id,
            or_(
                # Çalışan iş: kaydı uçurmak kullanıcıyı 404 poll'a düşürür.
                Analysis.status.not_in(TERMINAL_STATUSES),
                # Süresi dolmamış RAPOR: kullanıcının hakkı olan pencere.
                # `report IS NOT NULL` şart — iptal/hata kayıtlarında saklanacak
                # bir rapor yok, onların upload'ı süpürülmeye devam etmeli.
                and_(Analysis.report.is_not(None), Analysis.updated_at >= rapor_cutoff),
            ),
        )
        .exists()
    )
    uploads = list(
        (
            await session.scalars(select(Upload).where(Upload.updated_at < cutoff, ~korunan_analiz))
        ).all()
    )
