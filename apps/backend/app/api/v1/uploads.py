...
#:
#: Değer `core/body_limit.py`'den geliyor: aynı payı iki yerde tanımlamak,
#: birinin değişip diğerinin kalması demekti. Asıl sınır artık o middleware'de
#: uygulanıyor; buradaki kontrol derinlik savunması olarak kaldı.
_MULTIPART_ENVELOPE_ALLOWANCE = MULTIPART_ENVELOPE_ALLOWANCE
...
async def create_upload(
    ...
    if (
        ...
        and int(declared) > MAX_UPLOAD_BYTES + _MULTIPART_ENVELOPE_ALLOWANCE
    ):
