...
describe("ProgressScreen — terminal durumlar", () => {
  ...
  it("maliyet sınırı aşımında ne yapılacağını söyleyen mesajı gösterir", async () => {
    // Faz 3'te eklenen ANALYSIS_COST_LIMIT_EXCEEDED kodunun VARLIK SEBEBİ
    ...
    getAnalysisJob.mockResolvedValue(
      job({
        status: "failed",
        error: problem({
          type: "/errors/analysis-cost-limit-exceeded",
    ...
    expect(await screen.findByText("Analiz tamamlanamadı")).toBeInTheDocument();
    expect(screen.getByText(/Maliyet sınırını yükseltin/)).toBeInTheDocument();
